...
def connect_to_database(path: str) -> DuckDBPyConnection:
    ...
    logging.info("Connecting to database: {path}")
...
def collect_query_paths(parent_dir: str) -> List[str]:
    """Collects all query paths from the parent directory."""
    ...
    logging.info(f"Found {len(sql_files)} sql scripts at locations {parent_dir}")

    return sorted(sql_files)


def read_query(path: str) -> str:
    """Reads the query from the given path."""
...
def setup_database(database_path: str, ddl_query_parent_dir: str) -> None:
    """Sets up the database by creating tables and loading data."""
...
def destroy_database(database_path: str) -> None:
    """Destroys the database by removing the file."""
...
def main():
    ...
    parser.add_argument("--ddl-query-parent-dir", type=str, help="Path to the directory containing the DDL queries")
